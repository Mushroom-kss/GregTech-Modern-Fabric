...
import static com.gregtechceu.gtceu.common.data.GTMaterials.TOOL_ITEMS;
...
public class GTItems {
            ...
            if (tagPrefix.doGenerateItem()) {
                for (Material material : GTRegistries.MATERIALS) {
                    if (tagPrefix.doGenerateItem(material)) {
                        String first = tagPrefix.invertedName ? toLowerCaseUnder(tagPrefix.name)
                                : material.getName();
                        String last = tagPrefix.invertedName ? material.getName()
                                : toLowerCaseUnder(tagPrefix.name);
                        builder.put(tagPrefix, material, REGISTRATE
                                .item(first + "_" + last,
                                        properties -> TagPrefixItem.create(
                                                properties, tagPrefix,
                                                material))
                                ...
                                .onRegister(item -> {
                                    switch (tagPrefix.name) {
                                        case "buzzSawBlade", "screwDriverTip",
                                                "drillHead",
                                                "chainSawHead",
                                                "wrenchTip",
                                                "turbineBlade" ->
                                            CompassNode.getOrCreate(
                                                    GTCompassSections.MATERIALS,
                                                    "tool_heads")
                                                    .addItem(() -> item);
                                        default ->
                                            CompassNode.getOrCreate(
                                                    ...
                                                    FormattingUtil.toLowerCaseUnderscore(
                                                            tagPrefix.name))
                                                    .iconIfNull(() -> new ItemStackTexture(
                                                            item))
                                                    .addTag(tagPrefix
                                                            .getItemParentTags());
                                    }
    ...
    public static void generateTools() {
        ...
        for (Material material : GTRegistries.MATERIALS.values()) {
            if (material.hasProperty(PropertyKey.TOOL)) {
                ...
                List<ResourceLocation> lower = tiers.values().stream()
                        .filter(low -> low.getB().getLevel() == tier.getLevel() - 1)
                        .map(Tuple::getA).toList();
                List<ResourceLocation> higher = tiers.values().stream()
                        .filter(high -> high.getB().getLevel() == tier.getLevel() + 1)
                        .map(Tuple::getA).toList();
                    ...
                    if (property.hasType(toolType)) {
                        TOOL_ITEMS.put(material, toolType,
                                (ItemProviderEntry<IGTTool>) (ItemProviderEntry<?>) REGISTRATE
                                        .item("%s_%s".formatted(tier.material
                                                .getName()
                                                .toLowerCase(Locale.ROOT),
                                                toolType.name),
                                                p -> toolType.constructor
                                                        .apply(toolType, tier,
                                                                material,
                                                                toolType.toolDefinition,
                                                                p)
                                                        .asItem())
                                        .properties(p -> p.craftRemainder(
                                                Items.AIR))
    ...
    public static ItemEntry<Item> CREDIT_CUPRONICKEL = REGISTRATE.item("cupronickel_credit", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.MISC, "credit"))
            .register();
    ...
    static {
        ...
        SHAPE_MOLDS[0] = SHAPE_MOLD_PLATE = REGISTRATE.item("plate_casting_mold", Item::new)
                ...
                .onRegister(compassNodeExist(GTCompassSections.MISC, "mold"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 4))))
                .register();
        ...
        SHAPE_EXTRUDERS[8] = SHAPE_EXTRUDER_PIPE_SMALL = REGISTRATE.item("small_pipe_extruder_mold", Item::new)
                ...
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 4))))
                .register();
        SHAPE_EXTRUDERS[9] = SHAPE_EXTRUDER_PIPE_NORMAL = REGISTRATE
                .item("normal_pipe_extruder_mold", Item::new)
    ...
    public static ItemEntry<ComponentItem> SPRAY_SOLVENT = REGISTRATE
            .item("solvent_spray_can", ComponentItem::create)
    ...
    public static ItemEntry<ComponentItem> FLUID_CELL = REGISTRATE.item("fluid_cell", ComponentItem::create)
            ...
            .onRegister(modelPredicate(GTCEu.id("fluid_cell"),
                    (itemStack) -> FluidTransferHelper.getFluidContained(itemStack) == null ? 0f
                            : 1f))
            .onRegister(
                    attach(ThermalFluidStats.create((int) FluidHelper.getBucket(), 1800, true,
    ...
    public static ItemEntry<ComponentItem> FLUID_CELL_UNIVERSAL = REGISTRATE
            ...
            .onRegister(attach(cellName(),
                    ThermalFluidStats.create((int) FluidHelper.getBucket(), 1800, true, false,
                            false, false, true),
    ...
    public static ItemEntry<ComponentItem> ENERGIUM_CRYSTAL = REGISTRATE
            .item("energy_crystal", ComponentItem::create)
            ...
            .tag(CustomTags.HV_BATTERIES).register();
    public static ItemEntry<ComponentItem> LAPOTRON_CRYSTAL = REGISTRATE
            .item("lapotron_crystal", ComponentItem::create)
            .model(overrideModel(GTCEu.id("battery"), 8))
    ...
    public static ItemEntry<Item> ELECTRIC_MOTOR_MV = REGISTRATE.item("mv_electric_motor", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.MISC, "electric_motor")).register();
    ...
    public static ItemEntry<Item> ELECTRIC_MOTOR_UV = REGISTRATE.item("uv_electric_motor", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
            .register();
    public static ItemEntry<Item> ELECTRIC_MOTOR_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_electric_motor", Item::new).lang("UHV Electric Motor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
                    .register()
            : null;
    public static ItemEntry<Item> ELECTRIC_MOTOR_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_electric_motor", Item::new).lang("UEV Electric Motor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
                    .register()
            : null;
    public static ItemEntry<Item> ELECTRIC_MOTOR_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_electric_motor", Item::new).lang("UIV Electric Motor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
                    .register()
            : null;
    public static ItemEntry<Item> ELECTRIC_MOTOR_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_electric_motor", Item::new).lang("UXV Electric Motor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
                    .register()
            : null;
    public static ItemEntry<Item> ELECTRIC_MOTOR_OpV = GTCEu.isHighTier()
            ? REGISTRATE.item("opv_electric_motor", Item::new).lang("OpV Electric Motor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "electric_motor"))
                    .register()
            : null;

    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_LV = REGISTRATE
            .item("lv_electric_pump", ComponentItem::create)
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_MV = REGISTRATE
            .item("mv_electric_pump", ComponentItem::create)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "pump", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_HV = REGISTRATE
            .item("hv_electric_pump", ComponentItem::create)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "pump", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_EV = REGISTRATE
            .item("ev_electric_pump", ComponentItem::create)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "pump", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_IV = REGISTRATE
            .item("iv_electric_pump", ComponentItem::create)
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_ZPM = REGISTRATE
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "pump", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_UV = REGISTRATE
            .item("uv_electric_pump", ComponentItem::create)
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_UHV = GTCEu.isHighTier()
                    ...
                    .onRegister(attach(new TooltipBehavior(lines -> {
                        ...
                        lines.add(
                                Component.translatable(
                                        "gtceu.universal.tooltip.fluid_transfer_rate",
                                        1280 * 64 * 64 * 4 / 20));
                    })))
                    .onRegister(compassNodeExist(GTCompassSections.COVERS, "pump",
                            GTCompassNodes.COVER))
                    .register()
            : null;

    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_UEV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_UIV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_UXV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> ELECTRIC_PUMP_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_UV = REGISTRATE
            ...
            .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "fluid_regulator"))
            .register();
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_fluid_regulator", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_fluid_regulator", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_fluid_regulator", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_fluid_regulator", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> FLUID_REGULATOR_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_EV = REGISTRATE
            ...
            .onRegister(attach(new TooltipBehavior(lines -> {
                lines.add(Component.translatable("item.gtceu.conveyor.module.tooltip"));
                lines.add(Component.translatable("gtceu.universal.tooltip.item_transfer_rate_stacks",
                        3));
            })))
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "conveyor", GTCompassNodes.COVER))
            .register();
    ...
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_UV = REGISTRATE
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "conveyor", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_conveyor_module", ComponentItem::create)
                    ...
                    .onRegister(attach(new TooltipBehavior(lines -> {
                        ...
                        lines.add(Component.translatable(
                                "gtceu.universal.tooltip.item_transfer_rate_stacks",
                                16));
                    })))
                    .onRegister(compassNodeExist(GTCompassSections.COVERS, "conveyor",
                            GTCompassNodes.COVER))
                    .register()
            : null;
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_conveyor_module", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_conveyor_module", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_conveyor_module", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> CONVEYOR_MODULE_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<Item> ELECTRIC_PISTON_LV = REGISTRATE.item("lv_electric_piston", Item::new)
            .lang("LV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    public static ItemEntry<Item> ELECTRIC_PISTON_MV = REGISTRATE.item("mv_electric_piston", Item::new)
            .lang("MV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    public static ItemEntry<Item> ELECTRIC_PISTON_HV = REGISTRATE.item("hv_electric_piston", Item::new)
            .lang("HV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    public static ItemEntry<Item> ELECTRIC_PISTON_EV = REGISTRATE.item("ev_electric_piston", Item::new)
            .lang("EV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    public static ItemEntry<Item> ELECTRIC_PISTON_IV = REGISTRATE.item("iv_electric_piston", Item::new)
            .lang("IV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    ...
    public static ItemEntry<Item> ELECTRIC_PISTON_UV = REGISTRATE.item("uv_electric_piston", Item::new)
            .lang("UV Electric Piston").onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston"))
            .register();
    public static ItemEntry<Item> ELECTRIC_PISTON_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_electric_piston", Item::new).lang("UHV Electric Piston")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston")).register()
            : null;
    public static ItemEntry<Item> ELECTRIC_PISTON_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_electric_piston", Item::new).lang("UEV Electric Piston")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston")).register()
            : null;
    public static ItemEntry<Item> ELECTRIC_PISTON_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_electric_piston", Item::new).lang("UIV Electric Piston")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston")).register()
            : null;
    public static ItemEntry<Item> ELECTRIC_PISTON_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_electric_piston", Item::new).lang("UXV Electric Piston")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "piston")).register()
            : null;
    public static ItemEntry<Item> ELECTRIC_PISTON_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<ComponentItem> ROBOT_ARM_UV = REGISTRATE.item("uv_robot_arm", ComponentItem::create)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "robot_arm", GTCompassNodes.COVER))
            .register();
    public static ItemEntry<ComponentItem> ROBOT_ARM_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_robot_arm", ComponentItem::create)
                    ...
                    .onRegister(compassNodeExist(GTCompassSections.COVERS, "robot_arm",
                            GTCompassNodes.COVER))
                    .register()
            : null;
    public static ItemEntry<ComponentItem> ROBOT_ARM_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_robot_arm", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> ROBOT_ARM_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_robot_arm", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> ROBOT_ARM_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_robot_arm", ComponentItem::create)
                    ...
                    .register()
            : null;
    public static ItemEntry<ComponentItem> ROBOT_ARM_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<Item> FIELD_GENERATOR_UV = REGISTRATE.item("uv_field_generator", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "field_generator"))
            .register();
    public static ItemEntry<Item> FIELD_GENERATOR_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_field_generator", Item::new).lang("UHV Field Generator")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "field_generator"))
                    .register()
            : null;
    public static ItemEntry<Item> FIELD_GENERATOR_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_field_generator", Item::new).lang("UEV Field Generator")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "field_generator"))
                    .register()
            : null;
    public static ItemEntry<Item> FIELD_GENERATOR_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_field_generator", Item::new).lang("UIV Field Generator")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "field_generator"))
                    .register()
            : null;
    public static ItemEntry<Item> FIELD_GENERATOR_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_field_generator", Item::new).lang("UXV Field Generator")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "field_generator"))
                    .register()
            : null;
    public static ItemEntry<Item> FIELD_GENERATOR_OpV = GTCEu.isHighTier()
    ...
    public static ItemEntry<Item> SENSOR_UV = REGISTRATE.item("uv_sensor", Item::new).lang("UV Sensor")
            .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register();
    public static ItemEntry<Item> SENSOR_UHV = GTCEu.isHighTier()
            ? REGISTRATE.item("uhv_sensor", Item::new).lang("UHV Sensor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register()
            : null;
    public static ItemEntry<Item> SENSOR_UEV = GTCEu.isHighTier()
            ? REGISTRATE.item("uev_sensor", Item::new).lang("UEV Sensor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register()
            : null;
    public static ItemEntry<Item> SENSOR_UIV = GTCEu.isHighTier()
            ? REGISTRATE.item("uiv_sensor", Item::new).lang("UIV Sensor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register()
            : null;
    public static ItemEntry<Item> SENSOR_UXV = GTCEu.isHighTier()
            ? REGISTRATE.item("uxv_sensor", Item::new).lang("UXV Sensor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register()
            : null;
    public static ItemEntry<Item> SENSOR_OpV = GTCEu.isHighTier()
            ? REGISTRATE.item("opv_sensor", Item::new).lang("OpV Sensor")
                    .onRegister(compassNodeExist(GTCompassSections.COMPONENTS, "sensor")).register()
            : null;

    public static ItemEntry<Item> TOOL_DATA_STICK = REGISTRATE.item("data_stick", Item::new).lang("Data Stick")
            .onRegister(compassNode(GTCompassSections.COMPONENTS)).register();
    public static ItemEntry<Item> TOOL_DATA_ORB = REGISTRATE.item("data_orb", Item::new).lang("Data Orb")
            .onRegister(compassNode(GTCompassSections.COMPONENTS)).register();
    ...
    static {
        for (int i = 0; i < MarkerMaterials.Color.VALUES.length; i++) {
            ...
            if (color != MarkerMaterials.Color.White) {
                GLASS_LENSES.put(color, REGISTRATE
                        .item(String.format("%s_glass_lens", color.toString()), Item::new)
    ...
    public static ItemEntry<Item> NAQUADAH_WAFER = REGISTRATE.item("naquadah_wafer", Item::new)
            .lang("Naquadah-doped Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    ...
    public static ItemEntry<Item> NANO_CENTRAL_PROCESSING_UNIT_WAFER = REGISTRATE.item("nano_cpu_wafer", Item::new)
            .lang("Nano CPU Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> QUBIT_CENTRAL_PROCESSING_UNIT_WAFER = REGISTRATE.item("qbit_cpu_wafer", Item::new)
            .lang("Qubit CPU Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> SIMPLE_SYSTEM_ON_CHIP_WAFER = REGISTRATE.item("simple_soc_wafer", Item::new)
            .lang("Simple SoC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    ...
    public static ItemEntry<Item> ADVANCED_SYSTEM_ON_CHIP_WAFER = REGISTRATE.item("advanced_soc_wafer", Item::new)
            .lang("ASoC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> HIGHLY_ADVANCED_SOC_WAFER = REGISTRATE
            .item("highly_advanced_soc_wafer", Item::new)
            .lang("HASoC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> NAND_MEMORY_CHIP_WAFER = REGISTRATE.item("nand_memory_wafer", Item::new)
            .lang("NAND Memory Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> NOR_MEMORY_CHIP_WAFER = REGISTRATE.item("nor_memory_wafer", Item::new)
            .lang("NOR Memory Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> ULTRA_LOW_POWER_INTEGRATED_CIRCUIT_WAFER = REGISTRATE
            .item("ulpic_wafer", Item::new)
            .lang("ULPIC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> LOW_POWER_INTEGRATED_CIRCUIT_WAFER = REGISTRATE.item("lpic_wafer", Item::new)
            .lang("LPIC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> POWER_INTEGRATED_CIRCUIT_WAFER = REGISTRATE.item("mpic_wafer", Item::new)
            .lang("MPIC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> HIGH_POWER_INTEGRATED_CIRCUIT_WAFER = REGISTRATE.item("hpic_wafer", Item::new)
            .lang("HPIC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    public static ItemEntry<Item> ULTRA_HIGH_POWER_INTEGRATED_CIRCUIT_WAFER = REGISTRATE
            .item("uhpic_wafer", Item::new)
            .lang("UHPIC Wafer").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "wafer"))
            .register();
    ...
    public static ItemEntry<Item> ENGRAVED_LAPOTRON_CHIP = REGISTRATE
            .item("engraved_lapotron_crystal_chip", Item::new)
            .lang("Engraved Lapotron Crystal Chip").onRegister(compassNode(GTCompassSections.MISC))
            .register();
    ...
    public static ItemEntry<Item> NANO_CENTRAL_PROCESSING_UNIT = REGISTRATE.item("nano_cpu_chip", Item::new)
            .lang("Nano CPU Chip").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "cpus"))
            .register();
    public static ItemEntry<Item> QUBIT_CENTRAL_PROCESSING_UNIT = REGISTRATE.item("qbit_cpu_chip", Item::new)
            .lang("Qubit CPU Chip").onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "cpus"))
            .register();
    ...
    public static ItemEntry<Item> COATED_BOARD = REGISTRATE.item("resin_circuit_board", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "circuit_board"))
            .register();
    ...
    public static ItemEntry<Item> GOOD_CIRCUIT_BOARD = REGISTRATE.item("phenolic_printed_circuit_board", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "circuit_board")).register();
    public static ItemEntry<Item> PLASTIC_CIRCUIT_BOARD = REGISTRATE
            .item("plastic_printed_circuit_board", Item::new)
    ...
    public static ItemEntry<Item> EXTREME_CIRCUIT_BOARD = REGISTRATE
            ...
            .lang("Fiber-Reinforced Printed Circuit Board")
            .onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "circuit_board")).register();
    public static ItemEntry<Item> ELITE_CIRCUIT_BOARD = REGISTRATE
            ...
            .onRegister(compassNodeExist(GTCompassSections.CIRCUITS, "circuit_board")).register();
    public static ItemEntry<Item> WETWARE_CIRCUIT_BOARD = REGISTRATE
            .item("wetware_printed_circuit_board", Item::new)
    ...
    public static ItemEntry<Item> MICROPROCESSOR_LV = REGISTRATE.item("microchip_processor", Item::new)
            ...
            .onRegister(compassNode(GTCompassSections.CIRCUITS))
            .register();
    ...
    public static ItemEntry<Item> WETWARE_PROCESSOR_ASSEMBLY_ZPM = REGISTRATE
            ...
            .onRegister(compassNode(GTCompassSections.CIRCUITS)).register();
    public static ItemEntry<Item> WETWARE_SUPER_COMPUTER_UV = REGISTRATE
            .item("wetware_processor_computer", Item::new)
    ...
    public static ItemEntry<ComponentItem> COVER_INFINITE_WATER = REGISTRATE
            ...
            .onRegister(compassNode(GTCompassSections.COVERS, GTCompassNodes.COVER))
            .onRegister(attach(new TooltipBehavior(lines -> {
                lines.add(Component.translatable("gtceu.universal.tooltip.produces_fluid",
                        16_000 / 20));
    ...
    public static ItemEntry<ComponentItem> COVER_SOLAR_PANEL = REGISTRATE.item("solar_panel", ComponentItem::create)
            .lang("Solar Panel").onRegister(attach(new TooltipBehavior(lines -> {
                ...
                lines.add(Component.translatable("gtceu.universal.tooltip.voltage_out", 1,
    ...
    public static ItemEntry<ComponentItem> COVER_SOLAR_PANEL_ULV = REGISTRATE
            ...
            .onRegister(attach(new TooltipBehavior(lines -> {
                ...
                lines.add(Component.translatable("gtceu.universal.tooltip.voltage_out",
                        GTValues.V[GTValues.ULV],
                        GTValues.VNF[GTValues.ULV]));
            }))).onRegister(attach(new CoverPlaceBehavior(GTCovers.SOLAR_PANEL[0])))
            .onRegister(compassNodeExist(GTCompassSections.COVERS, "solar_panel", GTCompassNodes.COVER))
            .register();
    ...
    // public static ItemEntry<ComponentItem> FOAM_SPRAYER =
    // REGISTRATE.item("foam_sprayer", ComponentItem::create).onRegister(attach(new
    // FoamSprayerBehavior()).setMaxStackSize(1);
    ...
    public static ItemEntry<ComponentItem> STICKY_RESIN = REGISTRATE.item("sticky_resin", ComponentItem::create)
            .lang("Sticky Resin").onRegister(compassNode(GTCompassSections.MISC)).onRegister(burnTime(200))
            .register();
    ...
    static {
        for (int i = 0; i < DyeColor.values().length; i++) {
            var dyeColor = DyeColor.values()[i];
            SPRAY_CAN_DYES[i] = REGISTRATE
                    .item("%s_dye_spray_can".formatted(dyeColor.getName()), ComponentItem::create)
                    ...
                    .onRegister(attach(
                            new ColorSprayBehaviour(() -> SPRAY_EMPTY.asStack(), 512, i)))
                    .register();
        }
    }
    ...
    // public static ArmorMetaItem<?>.ArmorMetaValueItem
    // QUANTUM_CHESTPLATE_ADVANCED;

    public static ItemEntry<Item> POWER_THRUSTER = REGISTRATE.item("power_thruster", Item::new)
            .properties(p -> p.rarity(Rarity.UNCOMMON)).onRegister(compassNode(GTCompassSections.MISC))
            .register();
    ...
    public static ItemEntry<Item> NAN_CERTIFICATE = REGISTRATE.item("nan_certificate", Item::new)
            ...
            .onRegister(compassNodeExist(GTCompassSections.MISC, "certificate_of_not_being_a_noob_anymore"))
            .register();

    public static ItemEntry<ComponentItem> FERTILIZER = REGISTRATE.item("fertilizer", ComponentItem::create)
            .onRegister(attach(new FertilizerBehavior())).onRegister(compassNode(GTCompassSections.MISC))
            .register();
    ...
    public static void init() {
        ...
        COIN_GOLD_ANCIENT = REGISTRATE.item("ancient_gold_coin", Item::new)
                .lang("Ancient Gold Coin").properties(p -> p.rarity(Rarity.RARE))
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Gold, GTValues.M / 4))))
                .register();
        COIN_DOGE = REGISTRATE.item("doge_coin", Item::new).lang("Doge Coin")
                .properties(p -> p.rarity(Rarity.EPIC))
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Brass, GTValues.M / 4))))
                .register();
        COIN_CHOCOLATE = REGISTRATE.item("chocolate_coin", Item::new)
                .lang("Chocolate Coin")
                ...
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Gold, GTValues.M / 4))))
                .register();
        COMPRESSED_CLAY = REGISTRATE.item("compressed_clay", Item::new)
                .lang("Compressed Clay")
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(
                        new ItemMaterialInfo(new MaterialStack(GTMaterials.Clay, GTValues.M))))
                .register();
        COMPRESSED_COKE_CLAY = REGISTRATE.item("compressed_coke_clay", Item::new)
                .lang("Compressed Coke Clay")
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(
                        new ItemMaterialInfo(new MaterialStack(GTMaterials.Clay, GTValues.M))))
                .register();
        COMPRESSED_FIRECLAY = REGISTRATE.item("compressed_fireclay", Item::new)
                .lang("Compressed Fireclay")
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Fireclay, GTValues.M))))
                .register();
        FIRECLAY_BRICK = REGISTRATE.item("firebrick", Item::new)
                .lang("Firebrick")
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Fireclay, GTValues.M))))
                .register();
        COKE_OVEN_BRICK = REGISTRATE.item("coke_oven_brick", Item::new)
        ...
        SHAPE_EMPTY = REGISTRATE.item("empty_mold", Item::new)
                .lang("Empty Mold")
                .onRegister(compassNode(GTCompassSections.MISC))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 4))))
        ...
        BATTERY_HULL_LV = REGISTRATE.item("lv_battery_hull", Item::new)
                .lang("Small Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.BatteryAlloy, GTValues.M))))
                .register();
        BATTERY_HULL_MV = REGISTRATE.item("mv_battery_hull", Item::new)
                .lang("Medium Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.BatteryAlloy, GTValues.M * 3))))
                .register();
        BATTERY_HULL_HV = REGISTRATE.item("hv_battery_hull", Item::new)
                .lang("Large Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.BatteryAlloy, GTValues.M * 9))))
                .register();
        BATTERY_HULL_SMALL_VANADIUM = REGISTRATE.item("ev_battery_hull", Item::new)
                .lang("Small Vanadium Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.BlueSteel, GTValues.M * 2))))
                .register();
        BATTERY_HULL_MEDIUM_VANADIUM = REGISTRATE.item("iv_battery_hull", Item::new)
                .lang("Medium Vanadium Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.RoseGold, GTValues.M * 6))))
                .register();
        BATTERY_HULL_LARGE_VANADIUM = REGISTRATE.item("luv_battery_hull", Item::new)
                .lang("Large Vanadium Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.RedSteel, GTValues.M * 18))))
                .register();
        BATTERY_HULL_MEDIUM_NAQUADRIA = REGISTRATE.item("zpm_battery_hull", Item::new)
                .lang("Medium Naquadria Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Europium, GTValues.M * 6))))
                .register();
        BATTERY_HULL_LARGE_NAQUADRIA = REGISTRATE.item("uv_battery_hull", Item::new)
                .lang("Large Naquadria Battery Hull")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "battery_hull"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Americium, GTValues.M * 18))))
                .register();

        ITEM_FILTER = REGISTRATE.item("item_filter", ComponentItem::create)
                .onRegister(attach(new ItemFilterBehaviour(SimpleItemFilter::loadFilter),
                        new CoverPlaceBehavior(GTCovers.ITEM_FILTER)))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Zinc, GTValues.M * 2),
                        new MaterialStack(GTMaterials.Steel, GTValues.M))))
                .onRegister(compassNode(GTCompassSections.COVERS, GTCompassNodes.COVER)).register();
        ORE_DICTIONARY_FILTER = REGISTRATE
                .item("item_tag_filter", ComponentItem::create)
                .lang("Item Tag Filter")
                .onRegister(
                        attach(new ItemFilterBehaviour(TagItemFilter::loadFilter),
                                new CoverPlaceBehavior(GTCovers.ITEM_FILTER)))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Zinc, GTValues.M * 2))))
                .onRegister(compassNode(GTCompassSections.COVERS, GTCompassNodes.COVER)).register();
        FLUID_FILTER = REGISTRATE.item("fluid_filter", ComponentItem::create)
                .onRegister(attach(new FluidFilterBehaviour(SimpleFluidFilter::loadFilter),
                        new CoverPlaceBehavior(GTCovers.FLUID_FILTER)))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Zinc, GTValues.M * 2))))
                .onRegister(compassNode(GTCompassSections.COVERS, GTCompassNodes.COVER)).register();
        TAG_FLUID_FILTER = REGISTRATE.item("fluid_tag_filter", ComponentItem::create)
                .lang("Fluid Tag Filter")
                .onRegister(attach(new FluidFilterBehaviour(TagFluidFilter::loadFilter),
                        new CoverPlaceBehavior(GTCovers.FLUID_FILTER)))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Zinc, GTValues.M * 3 / 2))))
                .onRegister(compassNode(GTCompassSections.COVERS, GTCompassNodes.COVER)).register();
        FLUID_CELL_LARGE_STEEL = REGISTRATE
                ...
                .onRegister(compassNodeExist(GTCompassSections.ITEMS, "empty_cell"))
                .onRegister(modelPredicate(GTCEu.id("fluid_cell"),
                        (itemStack) -> FluidTransferHelper.getFluidContained(itemStack) == null
                                ? 0f
                                : 1f))
                .onRegister(attach(cellName(),
                        ThermalFluidStats.create((int) FluidHelper.getBucket() * 8,
                                GTMaterials.Steel.getProperty(PropertyKey.FLUID_PIPE)
                ...
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 4))))
                .register();
        FLUID_CELL_LARGE_ALUMINIUM = REGISTRATE
                ...
                .onRegister(attach(cellName(),
                        ThermalFluidStats.create((int) FluidHelper.getBucket() * 32,
                                GTMaterials.Aluminium
                                        .getProperty(PropertyKey.FLUID_PIPE)
                                        .getMaxFluidTemperature(),
                                true, false,
                                false, false, true),
                        new ItemFluidContainer()))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Aluminium, GTValues.M * 4))))
                .register();
        FLUID_CELL_LARGE_STAINLESS_STEEL = REGISTRATE
                ...
                .onRegister(attach(cellName(),
                        ...
                        new ItemFluidContainer()))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.StainlessSteel, GTValues.M * 6))))
                .register();
        FLUID_CELL_LARGE_TITANIUM = REGISTRATE
                ...
                .onRegister(attach(cellName(),
                        ...
                        new ItemFluidContainer()))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.TungstenSteel, GTValues.M * 6))))
                .register();
        FLUID_CELL_LARGE_TUNGSTEN_STEEL = REGISTRATE
                ...
                .onRegister(attach(cellName(),
                        ...
                        new ItemFluidContainer()))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.TungstenSteel, GTValues.M * 8))))
                .register();
        FLUID_CELL_GLASS_VIAL = REGISTRATE.item("glass_vial", ComponentItem::create)
                .model(GTModels::cellModel)
                .color(() -> GTItems::cellColor)
                .onRegister(compassNode(GTCompassSections.ITEMS))
                .onRegister(modelPredicate(GTCEu.id("fluid_cell"),
                        (itemStack) -> FluidTransferHelper.getFluidContained(itemStack) == null
                                ...
                                : 1f))
                .onRegister(attach(cellName(),
                        ThermalFluidStats.create((int) FluidHelper.getBucket(), 1200, false,
                                true, false, false, true),
                        new ItemFluidContainer()))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Glass, GTValues.M / 4))))
                .register();

        COMPONENT_GRINDER_DIAMOND = REGISTRATE.item("diamond_grinding_head", Item::new)
                .lang("Diamond Grinding Head")
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 8),
                        new MaterialStack(GTMaterials.Diamond, GTValues.M * 5))))
                .onRegister(compassNode(GTCompassSections.MISC)).register();
        COMPONENT_GRINDER_TUNGSTEN = REGISTRATE.item("tungsten_grinding_head", Item::new)
                .lang("Tungsten Grinding Head")
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Tungsten, GTValues.M * 4),
                        new MaterialStack(GTMaterials.VanadiumSteel, GTValues.M * 8),
                        new MaterialStack(GTMaterials.Diamond, GTValues.M))))
                .onRegister(compassNode(GTCompassSections.MISC)).register();

        VOLTAGE_COIL_ULV = REGISTRATE.item("ulv_voltage_coil", ComponentItem::create)
                .lang("Ultra Low Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Lead, GTValues.M * 2),
                        new MaterialStack(GTMaterials.IronMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_LV = REGISTRATE.item("lv_voltage_coil", ComponentItem::create)
                .lang("Low Voltage Coil").onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Steel, GTValues.M * 2),
                        new MaterialStack(GTMaterials.IronMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_MV = REGISTRATE.item("mv_voltage_coil", ComponentItem::create)
                .lang("Medium Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Aluminium, GTValues.M * 2),
                        new MaterialStack(GTMaterials.SteelMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_HV = REGISTRATE.item("hv_voltage_coil", ComponentItem::create)
                .lang("High Voltage Coil").onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.BlackSteel, GTValues.M * 2),
                        new MaterialStack(GTMaterials.SteelMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_EV = REGISTRATE.item("ev_voltage_coil", ComponentItem::create)
                .lang("Extreme Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.TungstenSteel, GTValues.M * 2),
                        new MaterialStack(GTMaterials.NeodymiumMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_IV = REGISTRATE.item("iv_voltage_coil", ComponentItem::create)
                .lang("Insane Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Iridium, GTValues.M * 2),
                        new MaterialStack(GTMaterials.NeodymiumMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_LuV = REGISTRATE.item("luv_voltage_coil", ComponentItem::create)
                .lang("Ludicrous Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Osmiridium, GTValues.M * 2),
                        new MaterialStack(GTMaterials.SamariumMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_ZPM = REGISTRATE.item("zpm_voltage_coil", ComponentItem::create)
                .lang("Zero Point Module Voltage Coil")
                .onRegister(compassNodeExist(GTCompassSections.MISC, "coil"))
                .onRegister(materialInfo(new ItemMaterialInfo(
                        new MaterialStack(GTMaterials.Europium, GTValues.M * 2),
                        new MaterialStack(GTMaterials.SamariumMagnetic, GTValues.M / 2))))
                .register();
        VOLTAGE_COIL_UV = REGISTRATE.item("uv_voltage_coil", ComponentItem::create)
    ...
    public static <P, T extends Item, S2 extends ItemBuilder<T, P>> NonNullFunction<S2, S2> unificationItem(
            @Nonnull TagPrefix tagPrefix, @Nonnull Material mat) {
        return builder -> {
                ...
                if (!world.isClientSide) {
                    ...
                    if (stackItem instanceof TagPrefixItem prefixItem) {
                        ...
                        player.setItemInHand(hand,
                                ChemicalHelper.get(purifyMap.get(prefixItem.tagPrefix),
    ...
    public static <T extends Item> NonNullConsumer<T> modelPredicate(ResourceLocation predicate,
        ...
        return item -> {
            if (LDLib.isClient()) {
                ItemProperties.register(item, predicate,
                        (itemStack, c, l, i) -> property.apply(itemStack));
            }
        };
    }

    public static void registerToolTier(MaterialToolTier tier, ResourceLocation id,
            Collection<ResourceLocation> before,
    ...
    private static <T extends Item> NonNullBiConsumer<DataGenContext<Item, T>, RegistrateLangProvider> reverseLangValue() {
        return (ctx, prov) -> {
            ...
            prov.add(ctx.get(),
                    names.stream().map(StringUtils::capitalize).collect(Collectors.joining(" ")));
        };
    }
